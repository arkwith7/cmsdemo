...
ENV DJANGO_SETTINGS_MODULE=mysite.settings.production DJANGO_DEBUG=off

# Tell uWSGI where to find your wsgi file:
ENV UWSGI_WSGI_FILE=mysite/wsgi.py
...
ENV UWSGI_STATIC_MAP="/media/=/code/mysite/media/"
...
RUN mkdir -p /code/mysite/media/images && chown -R 1000:2000 /code/mysite/media

# mark the destination for images as a volume
VOLUME ["/code/mysite/media/images/"]
