...
ENV DJANGO_SETTINGS_MODULE=inark.settings.production DJANGO_DEBUG=off

# Tell uWSGI where to find your wsgi file:
ENV UWSGI_WSGI_FILE=inark/wsgi.py
...
ENV UWSGI_STATIC_MAP="/media/=/code/inark/media/"
...
RUN mkdir -p /code/inark/media/images && chown -R 1000:2000 /code/inark/media

# mark the destination for images as a volume
VOLUME ["/code/inark/media/images/"]
